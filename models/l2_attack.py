...
class CarliniL2:
    def __init__(self, sess, model, batch_size=1, confidence=CONFIDENCE,
        ...
        if normalize_output:
            self.output /= tf.expand_dims(tf.reduce_sum(tf.abs(self.output), axis=1), -1)
    ...
    def attack(self, imgs, targets):
        ...
        """
        r = []
        print('go up to', len(imgs), flush=True)
    ...
    def attack_batch(self, imgs, labs):
        ...
        for outer_step in range(self.BINARY_SEARCH_STEPS):
            ...
            for iteration in range(self.MAX_ITERATIONS):
                ...
                if iteration % (self.MAX_ITERATIONS // 10) == 0:
                    print(iteration, self.sess.run((self.loss, self.loss1, self.loss2)), flush=True)
        ...
        print('best:', o_bestl2, flush=True)
        print('const:', CONST, flush=True)
        return o_bestattack